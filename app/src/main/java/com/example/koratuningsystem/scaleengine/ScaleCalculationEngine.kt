...
import com.example.koratuningsystem.scaleengine.model.ScaleCalculationResult
import com.example.koratuningsystem.scaleengine.model.ScaleStringRole
...
class ScaleCalculationEngine {
    ...
    fun calculate(request: ScaleCalculationRequest): ScaleCalculationResult {
        ...
        val leverRows = request.instrumentProfile.strings.map { string ->
            val role = roleForStringNumber(string.stringNumber)
        ...
        val pegRows = request.instrumentProfile.strings.map { string ->
            val role = roleForStringNumber(string.stringNumber)
            val options = buildLeverOptions(
                openPitch = string.openPitch,
                closedPitch = string.closedPitch,
    ...
    private fun roleForStringNumber(stringNumber: Int): ScaleStringRole {
        return if (stringNumber % 2 == 1) {
            ScaleStringRole(
                side = StringSide.LEFT,
                positionFromLow = (stringNumber / 2) + 1
            )
        } else {
            ScaleStringRole(
                side = StringSide.RIGHT,
